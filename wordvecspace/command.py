...
class WordVecSpaceCommand(BaseScript):
    ...
    def interact(self):
        if self.args.type == "mem":
            ...
            interact = WordVecSpaceAnnoy(
                self.args.input_dir, metric=self.args.metric, **eargs
            )
    ...
    def define_subcommands(self, subcommands):
        ...
        convert_cmd = subcommands.add_parser(
            "convert",
        ...
        convert_cmd.add_argument(
            "-v",
            "--vecs_per_shard",
            default=self.DEFAULT_VECS_PER_SHARD,
            type=int,
        ...
        convert_cmd.add_argument(
            "-s",
            "--shard_name",
            default=self.DEFAULT_SHARD_NAME,
            type=str,
        ...
        convert_cmd.add_argument(
            "-f",
            "--full_name",
            default=self.DEFAULT_FULL_NAME,
            type=str,
            help="Full name to map the tokens",
        )

        interact_cmd = subcommands.add_parser("interact", help="WordVecSpace Console")
        interact_cmd.set_defaults(func=self.interact)
        interact_cmd.add_argument(
            "type", help="wordvecspace feature mem or annoy or disk"
        )
        interact_cmd.add_argument("input_dir", help="wordvecspace input dir")
        interact_cmd.add_argument(
            "-m",
            "--metric",
            default=self.METRIC,
        ...
        runserver_cmd = subcommands.add_parser("runserver", help="WordVecSpace Service")
        runserver_cmd.set_defaults(func=self.runserver)
        runserver_cmd.add_argument(
            "type", help="wordvecspace feature mem or annoy or disk"
        )
        runserver_cmd.add_argument("input_dir", help="wordvecspace input directory")
        runserver_cmd.add_argument(
            "-m",
            "--metric",
            default=self.METRIC,
