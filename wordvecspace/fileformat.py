...
class WordVecSpaceFile(object):
    ...
    def _prepare_word_index_wvspace(self, dim, initialize=False, mode="r+"):
        ...
        m_array = DiskArray(
            m_path, dtype="float32", mode=mode, growby=self._growby, log=self.log
        )
        o_array = DiskArray(
            o_path, dtype="uint64", mode=mode, growby=self._growby, log=self.log
        )

        if not initialize:
            v_array = DiskArray(
                v_path, dtype="float32", mode=mode, growby=self._growby, log=self.log
            )
