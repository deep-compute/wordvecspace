...
class APIFunctions(object):
    def __init__(self, _type, input_dir, n_trees, metric, index_fpath):
        ...
        if self._type == "mem":
            ...
            self.wv = WordVecSpaceAnnoy(
                input_dir, n_trees=n_trees, metric=metric, index_fpath=index_fpath
            )
    ...
    def get_vector(
        self, word_or_index: Union[str, int], normalized: bool = False
    ) -> list:
        """
    ...
    def get_vectors(
        self, words_or_indices: Union[list, tuple], normalized: bool = False
    ) -> list:
        """
    ...
    def get_distances(
        ...
        return self.wv.get_distances(
            row_words_or_indices, col_words_or_indices=c
        ).tolist()
...
class WordVecSpaceServer(object):
    ...
    def start(self):
        ...
        self.api.register(
            APIFunctions(
                self._type, self.input_dir, self.n_trees, self.metric, self.index_fpath
            ),
            "v1",
        )
